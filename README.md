# System Architecture Diagram

## 🏗️ Hybrid Surat Management System Architecture

```
┌────────────────────────────────────────────────────────────────────┐
│                         CLIENT APPLICATIONS                         │
├───────────────────────────────┬────────────────────────────────────┤
│                               │                                     │
│    📱 Mobile App              │     💻 Web Admin Panel             │
│    (React Native Expo)        │     (Next.js)                      │
│                               │                                     │
│    Users: Pimpinan, Staf      │     Users: Admin Only              │
│                               │                                     │
│    Features:                  │     Features:                      │
│    • OAuth Login              │     • OAuth Login                  │
│    • View Letters             │     • User Management              │
│    • View Dispositions        │     • Team Management              │
│    • Team Chat (Read)         │     • Letter CRUD                  │
│    • PDF Viewer               │     • Send Dispositions            │
│    • Push Notifications       │     • Team Chat (Send)             │
│                               │     • Reports                      │
└───────────────────────────────┴────────────────────────────────────┘
                                ▼
                       HTTPS / REST API
                                ▼
┌────────────────────────────────────────────────────────────────────┐
│                         🚀 EXPRESS.JS API                           │
│                        (Node.js Backend)                            │
├────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  ┌──────────────────────────────────────────────────────────────┐ │
│  │                    🔐 Authentication Layer                    │ │
│  ├──────────────────────────────────────────────────────────────┤ │
│  │  • Clerk Authentication Platform                             │ │
│  │    - Universal Auth (Google, Email, etc)                     │ │
│  │    - JWT Validation via Clerk Middleware                     │ │
│  │    - Session & User Management by Clerk                      │ │
│  └──────────────────────────────────────────────────────────────┘ │
│                                                                     │
│  ┌──────────────────────────────────────────────────────────────┐ │
│  │                   🛡️ Authorization Layer                      │ │
│  ├──────────────────────────────────────────────────────────────┤ │
│  │  • RBAC Middleware                                           │ │
│  │  • Role Validation (ADMIN, PIMPINAN, STAF)                   │ │
│  │  • Resource Ownership Checks                                 │ │
│  └──────────────────────────────────────────────────────────────┘ │
│                                                                     │
│  ┌──────────────────────────────────────────────────────────────┐ │
│  │                      📋 API Routes                            │ │
│  ├──────────────────────────────────────────────────────────────┤ │
│  │  /auth/*           - OAuth & Authentication                  │ │
│  │  /api/users/*      - User Management (CRUD)                  │ │
│  │  /api/teams/*      - Team Management (CRUD)                  │ │
│  │  /api/letters/*    - Letter Management (CRUD)                │ │
│  │  /api/dispositions/* - Disposition Management                │ │
│  │  /api/messages/*   - Team Chat                               │ │
│  └──────────────────────────────────────────────────────────────┘ │
│                                                                     │
│  ┌──────────────────────────────────────────────────────────────┐ │
│  │                    🔧 Utility Layers                          │ │
│  ├──────────────────────────────────────────────────────────────┤ │
│  │  • Error Handling                                            │ │
│  │  • Input Validation                                          │ │
│  │  • CORS Configuration                                        │ │
│  │  • Request Logging                                           │ │
│  └──────────────────────────────────────────────────────────────┘ │
│                                                                     │
└────────────────────────────────────────────────────────────────────┘
          ▼                    ▼                    ▼
┌─────────────────┐  ┌──────────────────┐  ┌───────────────────┐
│  🗄️ Database    │  │  ☁️ File Storage  │  │  🔔 Real-time     │
│                 │  │                  │  │                   │
│  PostgreSQL     │  │  Cloudinary      │  │  Pusher Channels  │
│  (Neon DB)      │  │                  │  │                   │
│                 │  │  • PDF Storage   │  │  • Chat Messages  │
│  📊 Prisma ORM  │  │  • Public URLs   │  │  • Notifications  │
│                 │  │  • 10MB limit    │  │  • Status Updates │
│  Models:        │  │  • CDN Delivery  │  │                   │
│  • User         │  │                  │  │  Channels:        │
│  • Team         │  │                  │  │  • team-{id}      │
│  • TeamMember   │  │                  │  │  • user-{id}      │
│  • Letter       │  │                  │  │  • letters        │
│  • Disposition  │  │                  │  │                   │
│  • TeamMessage  │  │                  │  │  Events:          │
│                 │  │                  │  │  • new-message    │
│                 │  │                  │  │  • new-disposition│
│                 │  │                  │  │  • chat-lock      │
│                 │  │                  │  │  • new-letter     │
└─────────────────┘  └──────────────────┘  └───────────────────┘
```

## 🔄 Data Flow Diagrams

### 1. Authentication Flow (Clerk)

```
┌──────────┐         ┌────────────┐         ┌──────────────┐         ┌──────────┐
│  Client  │────1───>│   Clerk    │────2───>│ Identity     │         │ Database │
│(Web/Mobile)│        │  Platform  │         │ Providers    │         │(Neon DB) │
└──────────┘         └────────────┘         └──────────────┘         └──────────┘
     ▲                    │ ▲                       │
     │                    │ │                       │
     │                3.User authenticates          │
     │                    │ │                       ▼
     │                    │ └──────4. Callback──────┘
     │                    │
     │                    5. Clerk issues JWT       │
     │                    │
     │                    6. Client stores JWT      │
     │                    │
     │                    7. API requests with JWT──┼─────>│ Express API │
     │                                              │      │ (Clerk Middleware)
     │                                              │      │
     │                                              │      │ 8. Validate JWT & Sync User
     │                                              │      │ 9. Access Database
     │                                              │      │10. Return Data
     │                                              │      │
     └──────────────────────────────────────────────┘

Steps:
1. Client authenticates via Clerk (Google, Email, etc)
2. Clerk handles OAuth/identity provider
3. Clerk issues JWT to client
4. Client includes JWT in API requests
5. Express API validates JWT via Clerk middleware
6. User data is synced/queried in database as needed
7. Data returned to client
```

### 2. Letter Creation with PDF Upload

```
┌──────────┐         ┌────────────┐         ┌──────────────┐         ┌──────────┐
│  Admin   │────1───>│  Express   │────3───>│  Cloudinary  │         │ Database │
│  Client  │         │   API      │         │              │         │          │
└──────────┘         └────────────┘         └──────────────┘         └──────────┘
     ▲                    │ ▲                       │                       │
     │                    │ │                       │                       │
     │              2. Validate                     │                       │
     │               JWT & Role                     │                       │
     │                    │ │                       ▼                       │
     │                    │ └───────────4. Upload PDF────────>│            │
     │                    │                         │                       │
     │                    │<────5. Return URL───────┘                       │
     │                    │                                                 │
     │                    6. Save Letter Data with PDF URL ────────────────>│
     │                    │                                                 │
     │                    │<──────────7. Letter Created──────────────────────┘
     │                    │
     │                    8. Trigger Pusher Event
     │                    │
     └────────9. Return Letter Data with PDF URL──────────┘

Steps:
1. Admin uploads letter with PDF file
2. Express validates JWT and admin role
3. Multer processes multipart form data
4. File buffer sent to Cloudinary
5. Cloudinary returns secure URL
6. Letter saved to database with PDF URL
7. Database confirms creation
8. Real-time event triggered
9. Client receives letter data
```

### 3. Disposition Creation & Notification

```
┌──────────┐         ┌────────────┐         ┌──────────────┐         ┌──────────┐
│  Admin/  │────1───>│  Express   │────4───>│    Pusher    │         │ Database │
│ Pimpinan │         │   API      │         │   Channels   │         │          │
└──────────┘         └────────────┘         └──────────────┘         └──────────┘
     ▲                    │ ▲                       │                       │
     │              2. Validate JWT                 │                       │
     │               & Check Role                   │                       │
     │                    │ │                       │                       │
     │            3. Create Disposition ────────────────────────────────────>│
     │                    │ │                       │                       │
     │                    │<────────────────────────────5. Disposition ID───┘
     │                    │                         │
     │                    └────6. Trigger Event────>│
     │                    │      (new-disposition)  │
     │                    │                         └────>7. Push to Client
     │                    │
     └────────8. Return Disposition Data──────────┘

Steps:
1. Admin/Pimpinan sends disposition to user/team
2. Express validates role (Admin or Pimpinan)
3. Disposition created in database
4. Pusher notified
5. Database confirms creation
6. Pusher event triggered
7. Real-time notification sent to recipient
8. Creator receives confirmation
```

### 4. Team Chat Message Flow

```
┌──────────┐         ┌────────────┐         ┌──────────────┐         ┌──────────┐
│  Admin   │────1───>│  Express   │────4───>│    Pusher    │         │ Database │
│  Client  │         │   API      │         │   Channels   │         │          │
└──────────┘         └────────────┘         └──────────────┘         └──────────┘
     ▲                    │ ▲                       │                       │
     │                    │ │                       │                       │
     │              2. Validate                     │                       │
     │              • JWT Token                     │                       │
     │              • Admin Role                    │                       │
     │              • Chat Not Locked               │                       │
     │                    │ │                       │                       │
     │            3. Save Message ──────────────────────────────────────────>│
     │                    │ │                       │                       │
     │                    │<──────────────────────────5. Message ID─────────┘
     │                    │                         │
     │                    └───6. Trigger Event─────>│
     │                    │     (new-message)       │
     │                    │                         └────>7. Broadcast
     │                    │                              to Team Members
     └────────8. Return Message Data──────────┘

                         ┌──────────┐  ┌──────────┐
                         │ Pimpinan │  │   Staf   │
                         │(Read-Only)│  │(Read-Only)│
                         └─────▲────┘  └─────▲────┘
                               │             │
                               └─────────────┴──── 7. Receive Message

Steps:
1. Admin sends message to team chat
2. Express validates: JWT, Admin role, chat unlocked
3. Message saved to database
4. Pusher notified
5. Database returns message ID
6. Pusher event triggered
7. Real-time broadcast to all team members
8. Admin receives confirmation
```

## 🎯 Role-Based Access Pattern

```
┌─────────────────────────────────────────────────────────────────┐
│                        Request Flow                              │
└─────────────────────────────────────────────────────────────────┘
                              │
                              ▼
                  ┌───────────────────────┐
                  │ 1. JWT Authentication │
                  │    (authenticateToken)│
                  └───────────┬───────────┘
                              │
                    ┌─────────▼─────────┐
                    │   Token Valid?    │
                    └─────────┬─────────┘
                         Yes  │  No
                    ┌─────────┴──────────┐
                    ▼                    ▼
        ┌───────────────────┐    ┌──────────────┐
        │ 2. Role Check     │    │ 401 Error    │
        │    (requireRole)  │    │ Unauthorized │
        └───────────┬───────┘    └──────────────┘
                    │
        ┌───────────▼────────────┐
        │   Has Required Role?   │
        └───────────┬────────────┘
             Yes    │    No
        ┌───────────┴────────┐
        ▼                    ▼
┌───────────────┐    ┌──────────────┐
│ 3. Execute    │    │ 403 Error    │
│    Route      │    │  Forbidden   │
│    Handler    │    └──────────────┘
└───────┬───────┘
        │
        ▼
┌───────────────┐
│ Return Data   │
└───────────────┘

Role Hierarchy:
┌──────────────────────────────────────────────┐
│ ADMIN                                        │
│ • All permissions                            │
│ • User management                            │
│ • Team management                            │
│ • Letter CRUD                                │
│ • Send messages                              │
│ • Lock/unlock chat                           │
└──────────────────────────────────────────────┘
         │ Can do everything PIMPINAN can
         ▼
┌──────────────────────────────────────────────┐
│ PIMPINAN                                     │
│ • View letters                               │
│ • Send dispositions                          │
│ • View team messages (read-only)             │
│ • Update disposition status                  │
└──────────────────────────────────────────────┘
         │ Can do everything STAF can
         ▼
┌──────────────────────────────────────────────┐
│ STAF                                         │
│ • View assigned letters                      │
│ • View their dispositions                    │
│ • View team messages (read-only)             │
│ • Update disposition status (their own)      │
└──────────────────────────────────────────────┘
```

## 🗄️ Database Relationships

```
┌─────────────────────────────────────────────────────────────────┐
│                     Database Schema                              │
└─────────────────────────────────────────────────────────────────┘

┌───────────────┐
│     User      │
│───────────────│
│ id (PK)       │
│ email         │◄────────────┐
│ name          │             │
│ role          │             │
│ provider      │             │
│ providerId    │             │
│ avatarUrl     │             │
└───────┬───────┘             │
        │                     │
        │ 1:N                 │ N:1
        │                     │
        ▼                     │
┌───────────────┐             │
│  TeamMember   │             │
│───────────────│             │
│ id (PK)       │             │
│ userId (FK)   │─────────────┘
│ teamId (FK)   │─────────────┐
└───────────────┘             │
                              │ N:1
                              │
                    ┌─────────▼─────┐
                    │     Team      │
                    │───────────────│
                    │ id (PK)       │
                    │ name          │
                    │ description   │
                    │ isChatLocked  │
                    └───────┬───────┘
                            │
                            │ 1:N
                            │
              ┌─────────────┴──────────────┐
              │                            │
              ▼                            ▼
    ┌──────────────────┐         ┌─────────────────┐
    │  TeamMessage     │         │  Disposition    │
    │──────────────────│         │─────────────────│
    │ id (PK)          │         │ id (PK)         │
    │ teamId (FK)──────┘         │ teamId (FK)─────┘
    │ senderId (FK)              │ letterId (FK)───┐
    │ content                    │ senderId (FK)   │
    │ createdAt                  │ recipientId(FK) │
    └──────────────────┘         │ status          │
                                 │ notes           │
                                 └────────┬────────┘
                                          │ N:1
                                          │
                                          ▼
                                 ┌────────────────┐
                                 │    Letter      │
                                 │────────────────│
                                 │ id (PK)        │
                                 │ type           │
                                 │ referenceNumber│
                                 │ subject        │
                                 │ sender         │
                                 │ recipient      │
                                 │ letterDate     │
                                 │ fileUrl        │
                                 │ description    │
                                 └────────────────┘

Relationships:
• User ←→ TeamMember ←→ Team (Many-to-Many)
• User ←─ TeamMessage (One-to-Many)
• Team ←─ TeamMessage (One-to-Many)
• User ←─ Disposition (One-to-Many, as sender)
• User ←─ Disposition (One-to-Many, as recipient)
• Team ←─ Disposition (One-to-Many)
• Letter ←─ Disposition (One-to-Many)
```

---

**Use these diagrams to understand the system architecture! 📊**
